...
# [3] 속성명과 테이블명의 별치 , 별칭이란? 조회 결과에만 적용되는 속성명 변경 , 원본 속성명은 그대로 , *다중테이블 조회시 속성명 중복 또는 간소화
...
select distinct maddr from member;			# maddr(주속) 속성값 중복 제거된 레코드 조회

# [5] 속성값 산술연산
# 산술연산자 : 속성명 + 값 , 속성명 - 값 , 속성명 * 값 , 속성명 / 값 , 속성명 div 값 , 속성명 mod 값
select mnumber 인원수 , mnumber + 3 더하기 , mnumber - 3 빼기  , mnumber * 3 , mnumber / 3 , 
...
select * from member where mname != '블랙핑크';					# 'mname'속성값이 '블랭핑크'가 아닌 레코드 조회
...
# 'maddr' 속성값이 '경기'이거나 'maddr'속성값이 '경기'이거나 'maddr'속성값이 '경남'인 레코드 조회
...
select * from member where mname like '%핑크';					# 'mname'속성값이 '핑크'로 끝나느 레코드 조회
...
# order by 속성명 asc(생략가능)	, 지정한 속성명 기준으로 속성값들을 오름차순 정렬 , 작은값 -> 큰값 , 과거날짜 -> 최근날짜 , ABC , ㄱㄴㄷ
# order by 속성명 desc		, 지정한 속성명 기준으로 속성값들을 내림차순 정렬 , 큰값 -> 작은값 , 최근날짜 -> 과거날짜 , CBA , ㄷㄴㄱ
# order by 속성명1 정렬기준 , 속성명2 정렬기준; , 지정한 속성명의 정렬이 2개 이상일때 , (쉼표) 구분하기 , 2차정렬은 1차 정렬내 동일한 값 내에서 2차정렬
...
# 먼저 'maddr'속성값을 정렬 후 동일한 속성값이 존재할 경우 동일한 속성값끼리 'mdebut' 정렬
select * from member order by maddr desc , mdebut asc;	

# [8] 제한 , 조회 결과의 레코드 수 제한
# limit 레코드수
# limit 시작레코드(0~) , 개수
select * from member limit 2;						# 조회 결과의 레코드에서 상위 2개의 결과만 조회 제한
select * from member limit 0 , 2;					# 조회 결과의 레코드에서 0(첫번째)레코드부터 2개만 조회 제한
select * from member limit 2 , 3;					# 조회 결과의 레코드에서 2(두번째)레코드부터 3개만 조회 제한
select * from member order by mheight desc limit 3;	# 'mheight'가 상위 3개미만 조회 제한

# 실습1 : 주소가 '서울'인 레코드를 데뷔일 기준으로 내림차순으로 조회 결과 상위 2개만 조회하시오.
...
select * from buy;									# 'buy' 테이블 저체 레코드 조회
select bpname from buy group by bpname;				# 'buy' 테이블의 'bpname'속성명 값을 그룹 , 중복 값이 없다.
select bpname , bprice from buy group by bpname;	# 오류 : bpname이 동일하더라고 bprice 다를 수 있으므로
/*
	학년		점수		1학년 총 점수합계
    1		30		
    2		20
...
# 1. 회원아이디별 구매수량 총합계 조회
select mid , sum( bamount ) from buy group by mid;	# 'mid' 속성명의 값으로 그룹(중복)하고 'bamount' 속성명을 집계한 레코드 조회
# 2. 회원아이디별 총구매금액 조회 , 구매수량 * 구매가격
...
# 주의할점 : select문에서 컴퓨터가 처리하는 순서 : from -> where -> 속성명 (select [3] from [1] where [2] group by [4] having [5]
# 1. 회원아이디별 구매수량( bamount )이 3 초과인 레코드 조회
select mid from buy where bamount > 3;
# 2. 회원아이디별 총구매금액( bamount * bprice )이 1000 이상인 레코드 조회
...
select mid , bprice 구매가격 from buy where 구매가격 >= 1000;	# 오류 : where절이 먼저 처리되기 때문에 구매가격을 모름 # where 별칭으로 조건속성명 불가능
select mid , sum( bamount * bprice ) as 총구매금액 where 총구매금액 >= 1000;	# 오류 : 집계 후 조건은 where 불가능

# 실습2 : 구매수량( bamount )이 2개 초과인 레코드에서 회원아이디별 구매가격( bprice )이 50 이상인 레코드 조회 
# + 구매가격평균으로 내림차순 , 상위 2개만 조회
# 1. buy 테이블에서 구매수량( bamount )이 2개 초과인 레코드
select * from buy where amount > 2;

# 2. 화원아이디( mid )별 구매가격( bprice )평균 * ~~별 집계(합계/평균/개수)
select mid as 회원아이디 , avg( bprice ) as 구매가격평균
from buy
where bamount > 2
group by 회원아이디;

# 3. 구매가격( bprice )평균이 50이상 , 구매가격평균은 집게인지 아닌지 판단 , 집계이면 having 아니면 where
select mid as 회원아이디 , avg( bprice ) as 구매가격평균
from buy
where bamount > 2
group by 회원아이디
...
# select [3] form [1] where [2] group by [4] having [5] order by [6] limit [7]
/*
