...
select * from table1;
select * from table2;
	# PK(기본키/식별키) 필드/속성 : 고유한 값들을 가진 필드			, 중복없음(unique)	, not null	, ex) 주민등록번호 , 사번 , 학번 , 제품번호 , 주문번호
	# FK(외래키/참조키) 필드/속성 : 다른 테이블의 속성값 참조하는 필드	, 중복허용			, null 허용	, pk 속성값을 참조하는 속성값

# 5. 집합 : 교집합 : 두 테이블간의 공통으로 포함하는 속성값으로 이루어진 테이블조회
# 조회시 조회단위 : 레코드 단위
select * from table1;				# 레코드 5개
select * from table1 , table2;		# 레코드 25개 , 2개 이상의 테이블을 조회 : select * from 테이블명A , 테이블명B;
# INNER JOIN
# [1] where 이용한 교집합 표현하기 , 두 테이블의 공통 테이블 num_pk와 num_pk(FK) , 주로 PK속성 - FK속성
...
# [4] using 키워드 : 전제조건은 pk속성명과 fk속성명이 같아야한다. 현재 예제는 num_pk/num_pk(FK)
select * from table1 join table2 using( num_pk );

# [5] nature join 키워드 : 두 테이블간의 동일한 속성명 기준으로 자동으로 조인한다
...
# [7] 오른쪽 테이블(table2)에 있는 모든 레코드를 포함하고 왼쪽 테이블(table1)에 교집합되는 레코드 조회 , 없으면 null
...
# 오라클DB : FULL OUTER JOIN 지원 , MYSQLDB : union 이용한 합집합

# [10] 차집합 : 두 집합에서 A에는 속하지만 B에는 속하지 않는 원소집합 , 두테이블에서 특정 테이블에 속하지만 다른 테이블에 속하지 않는 레코드 조회
...
# [문제1] : 모든 제품(product)테이블 레코드 조회시 카테고리번호 대신에 카테고리명을 조회하는 SQL 작성하시오.
select * from product;
select * from product , pcategory;
...
# 별칭
select p.제품번호_pk , p.제품명 , p.제품가격 , pc.카테고리명 from product p inner join pcategory pc on p.카테고리번호_fk = pc.카테고리번호_pk;

# [문제2] : 제품번호 1번의 재고수량 합계를 조회하는 SQL 작성하시오, 그리고 조회시 제품명과 합계수량 속성명으로 조회하시오.
select * from stock;
select * from stock where 제품번호_fk = 1;
select sum(재고수량) 합계수량 from stock where 제품번호_fk = 1;
# 두 테이블을 join
select p.제품명 , sum(s.재고수량) 합계수량 from stock s inner join product on s.제풉번호_fk  = p.제풉번호_pk where s.제품번호_fk = 1;
