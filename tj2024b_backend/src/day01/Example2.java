		...
		/*
		    - 자바 기본 타입 , 8가지
		 	...
		 	[문자]	char		2바이트		유니코드 , 작은따옴표로 감싼 문자		, 'A'
		 			*String		N바이트		문자열클래스 , 큰따옴표로 감싼 문자	, "ABC"
		 			
		 	[정수]	byte		1바이트		-128 ~ +127 저장 (0포함돼서 127)
		 			short		2바이트		+-3만정도 저장
		 			int			4바이트		+-21억정도 저장 , 리터럴정수(키보드로 입력한 정수) , 정수들 중에 기본타입
		 			long		8바이트		+-21억이상 저장 , 리터럴정수 뒤에 L/l 붙이기
		 			
		 	[실수 , 부동소수점]	
		 			float		4바이트		소수점8자리 포현
...
//		boolean bool2 = 3; 		// 오류발생 : boolean 타입에 3을 저장할 수 없다.
		...
		// 2. char 타입은 작은따옴표를 이용한 문자1개만 저장할 수 있다.
...
//		float f1 = 0.123456789123456789;	// 오류발생 : 실수 리터럴값은 double 이므로 float에 대입할 수 없다.
		...
		double d1 = 0.123456789123456789;	System.out.println("d1 : " + d1); // 0.12345678912345679
