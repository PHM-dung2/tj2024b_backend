...
//	왜/목적? 여러 사람들이 작업할때 클래스명에 dto 명시 함으로써 클래스 사용처 파익이 쉽다.
//		추후에 dto , vo , dao , controller , service , view , entity
//	DTO : 데이터베이스(DB)에 저장된 자료들을 자바로 이동할때 사용되는 객체
//		자바와 자바스크립트 휘발성(영구저장X)
//		자바/자바스크립트 <----> DB(영구저장O)
//	관례적으로 모든 멤버변수는 private , 디폴트생성자+풀매개변수 , setter/getter메소드 , toString메소드 :객체멤버변수 정보반환함수
...
//		private 멤버변수는 외부 클래스로부터 차단했으므로 간접적으로 접근할 수 있도록 public 메소드 제공
	public String getTiltle() { return this.title; }
...
//		toString() : 기본적으로 객체 호출하면 주소(참조)값이 반환되자만, 오버라이딩(재정의)해서 주소값 대신 원하는 반환값 정의
	@Override
