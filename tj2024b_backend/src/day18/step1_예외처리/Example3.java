...
//		예측 안되면 => 실행 => 예외확인 => 예외처리
//		static : 전역변수/메소드 : 프로그램이 실행될때 static키워드가 존재하는 멤버변수와 메소드는 메모리에 우선 할당되고 프로그램이 종료되면 사라진다.
//		--> 정적 : 메모리 고정 , static vs 동적 : 인스턴스 , new(생성) , GC(삭제->참조를 당하지 않으면)
//		--> 인스턴스가 없어도 이미 메모리(실체)가 존재하므로 바로 사용이 가능하다. static 없는 변수나 메소드는 인스턴스(메모리/실체)가 필요하다.
	}
	
	public static void main(String[] args) {
		
//		1. 메소드 호출 , 예외처리 발생 : 호출한 메소드가 throws 이용하여 메소드 내부 예외를 반환했기 떄문에
		try {
